...
# sources of the version declared in android-app/ort-adapter/build.gradle. This is an API-compatibility
...
VER=$(grep -oE "onnxRuntimeVersion = '[0-9.]+'" "$ROOT/android-app/ort-adapter/build.gradle" | grep -oE '[0-9.]+')
...
SRCS=$(find "$ORT_SRC/java/src/main/java" "$ORT_SRC/java/src/main/jvm" -name '*.java'; \
       ...
       find "$ROOT/android-app/core/src/main/java" "$ROOT/android-app/ort-adapter/src/main/java" "$ROOT/android-app/app/src/main/java" -name '*.java')
if [ -n "${ZS_ECJ:-}" ]; then
